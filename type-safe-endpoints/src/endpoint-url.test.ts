...
import { endpointUrl } from "./endpoint-url";
...
it("should return the parsed endpoint url", () => {
  ...
  assert.equal(
    endpointUrl("/payments/{id}/transaction/{tr_id}", {
      id: "ifh-4fd",
      tr_id: "jh7-31d",
    }),
  ...
  assert.equal(
    endpointUrl("https://forum.example.io/threads/{thread_id}", {
      thread_id: "jof-3fa",
    }),
  ...
  assert.equal(
    endpointUrl("https://example.com/users/{user_id}/posts/{post_id}", {
      user_id: "usr_1",
      post_id: "pst_1",
    }),
